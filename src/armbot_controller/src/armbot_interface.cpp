...
CallbackReturn ArmbotInterface::on_activate(const rclcpp_lifecycle::State &previous_state)
{
  RCLCPP_INFO(rclcpp::get_logger("ArmbotInterface"), "Starting robot hardware ...");
  ...
  RCLCPP_INFO(rclcpp::get_logger("ArmbotInterface"),
              "Hardware started, ready to take commands");
  return CallbackReturn::SUCCESS;
}


CallbackReturn ArmbotInterface::on_deactivate(const rclcpp_lifecycle::State &previous_state)
{
  RCLCPP_INFO(rclcpp::get_logger("ArmbotInterface"), "Stopping robot hardware ...");
  ...
  RCLCPP_INFO(rclcpp::get_logger("ArmbotInterface"), "Hardware stopped");
  return CallbackReturn::SUCCESS;
}


hardware_interface::return_type ArmbotInterface::read(const rclcpp::Time &time,
...
hardware_interface::return_type ArmbotInterface::write(const rclcpp::Time &time,
                                                           const rclcpp::Duration &period)
{
  ...
  msg.append(",");
  int wriste_pitch = static_cast<int>(((position_commands_.at(3) + (M_PI / 2)) * 180) / M_PI);
  msg.append("w");
  msg.append(std::to_string(wriste_pitch));
  msg.append(",");
  int wriste_roll = static_cast<int>(((position_commands_.at(4) + (M_PI / 2)) * 180) / M_PI);
  msg.append("r");
  msg.append(std::to_string(wriste_roll));
  msg.append(",");
  ...
  try
  {
    RCLCPP_INFO_STREAM(rclcpp::get_logger("ArmbotInterface"), "Sending new command " << msg);
    arduino_.Write(msg);
  }
  catch (...)
  {
