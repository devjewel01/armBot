...
def generate_launch_description():
    # Launch configuration for determining if it is a simulation
    is_sim = LaunchConfiguration("is_sim")

    # Declare the is_sim argument with a default value of "True"
    is_sim_arg = DeclareLaunchArgument(
        "is_sim",
        default_value="True",
        description="Set to 'True' if running in simulation mode, 'False' for real hardware"
    )
    ...
    # Controller manager node (active only in non-simulation mode)
    ...
    # Return a LaunchDescription with all necessary components
